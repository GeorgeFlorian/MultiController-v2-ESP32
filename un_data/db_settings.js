...
function get_system_info() {
  get_json("api/system/info")
    .then((json_data) => {
      console.log("Received settings: " + json_data);
      document.getElementById("about").innerHTML =
        "DroneBridge for ESP32 - v" +
        json_data["major_version"] +
        "." +
        json_data["minor_version"] +
        " - esp-idf " +
        json_data["idf_version"];
    })
    .catch((error) => {
      conn_status = 0;
      error.message;
...
function update_conn_status() {
  if (conn_status)
    document.getElementById("web_conn_status").innerHTML =
      '<span class="dot_green"></span> connected to ESP32';
  else
    document.getElementById("web_conn_status").innerHTML =
      '<span class="dot_red"></span> disconnected from ESP32';
}

function get_stats() {
  get_json("api/system/stats")
    .then((json_data) => {
      conn_status = 1;
      ...
      if (!isNaN(tcp_clients) && tcp_clients > 1) {
        document.getElementById("tcp_connected").innerHTML =
          tcp_clients + " clients";
      ...
      if (!isNaN(udp_clients) && udp_clients > 1) {
        document.getElementById("udp_connected").innerHTML =
          udp_clients + " clients";
